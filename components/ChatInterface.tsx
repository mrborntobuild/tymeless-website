...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ persona, onClose, fullPage = false }) => {
  ...
  // Full page layout for mobile
  if (fullPage) {
    return (
      <div className="min-h-screen flex flex-col bg-white">
        {/* Mobile Header with Back Button */}
        <div className="flex items-center gap-4 p-4 border-b border-gray-100 bg-white sticky top-0 z-10">
          <button 
            onClick={onClose}
            className="p-2 hover:bg-gray-100 rounded-full transition-colors"
            aria-label="Go back"
          >
            <ArrowLeft size={24} className="text-cradle-text" />
          </button>
          <div className="flex items-center gap-3 flex-1">
            ...
            />
            <div>
              <span className="font-serif font-semibold text-lg block">{persona.name}</span>
              <span className="text-xs text-gray-500">{persona.age}</span>
            </div>
          </div>
        </div>
  ...
  // Original modal layout for desktop
  return (
